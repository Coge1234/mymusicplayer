...
public class DiscView extends RelativeLayout {
    ...
    private boolean mIsNeedleStartPlayAnimator = false;
    ...
    private void initDiscBlackgroung() {
    ...
    private void doWithAnimatorOnPageScroll(int state) {
        switch (state) {
            //0 idle(空闲，挂空挡)， 理解为：只要拖动/滑动结束，无论是否安放到了目标页，
            // 则 state = ViewPager.SCROLL_STATE_IDLE
            case ViewPager.SCROLL_STATE_IDLE:
                //2 settling(安放、定居、解决)，理解为：通过拖动/滑动，安放到了目标页，
                // 则 state = ViewPager.SCROLL_STATE_SETTLING
            ...
            //1 dragging（拖动），理解为：只要触发拖动/滑动事件时，
            // 则 state = ViewPager.SCROLL_STATE_DRAGGING
            ...
            public void onAnimationEnd(Animator animation) {
                if (mNeedleAnimatorStatus == NeedleAnimatorStatus.TO_NEAR_END) {
                    ...
                    if (mMusicStatus == MusicStatus.STOP) {
                        mIsNeedleStartPlayAnimator = true;
                    }
                }
                if (mIsNeedleStartPlayAnimator) {
                    mIsNeedleStartPlayAnimator = false;
    ...
    public void setMusicDataList(List<MusicData> musicDataList) {
        ...
        if (null != mIPlayInfo) {
            mIPlayInfo.onMusicInfoChanged(musicData.getMusicName(), musicData.getMusicSinger());
